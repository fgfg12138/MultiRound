...
export default function Create() {
    ...
    return (
      <Layout title="创建圆桌" showBack backTo="/">
        <div className="max-w-md mx-auto px-4 py-16 text-center">
          <div className="w-16 h-16 bg-warning/20 rounded-r-xl flex items-center justify-center mx-auto mb-4">
  ...
  const SectionNum = ({ n }: { n: number }) => (
    <span className="w-7 h-7 rounded-full bg-p600 text-white flex items-center justify-center text-xs font-bold flex-shrink-0">{n}</span>
  );
      ...
      <div className="max-w-3xl mx-auto w-full px-4 py-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pb-24">
          ...
          <section className="space-y-3">
            ...
            <div>
              <label className="block text-xs text-g500 mb-1">场景描述</label>
              <textarea value={scenarioDesc} onChange={e => setScenarioDesc(e.target.value)} placeholder="描述讨论的背景、上下文和期望方向..." rows={3} className="w-full px-3 py-2 text-sm border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400 focus:border-transparent resize-none" />
              <button type="button"
                onClick={async () => {
                  try {
                    ...
                    if (draft.gameMode === 'werewolf') { setGameMode('werewolf'); setModNightAction(true); setModVote(true); setModDeathSilence(true); setModWinCheck(true); setModPhaseIndicator(true); }
                    if (draft.modules) { if (draft.modules.nightAction !== undefined) setModNightAction(draft.modules.nightAction); if (draft.modules.vote !== undefined) setModVote(draft.modules.vote); if (draft.modules.deathSilence !== undefined) setModDeathSilence(draft.modules.deathSilence); if (draft.modules.winCheck !== undefined) setModWinCheck(draft.modules.winCheck); if (draft.modules.phaseIndicator !== undefined) setModPhaseIndicator(draft.modules.phaseIndicator); }
                    ...
                    if (draft.characters && draft.characters.length > 0) {
                      setCharacters(draft.characters.map(function(cx) { var s = cx.secret || {}; return withDefaults({ id: generateId(), name: cx.name || '', role: cx.role || '', persona: cx.persona || '', stance: cx.stance || '', style: cx.style || '', motivation: cx.motivation, expertise: cx.expertise, relationship: cx.relationship, constraints: cx.constraints, teamId: cx.teamId, providerId: '', secret: { secretRole: (s.secretRole || 'normal') as any, publicGoal: s.publicGoal || '', privateGoal: s.privateGoal || '', knownSecrets: [] as any, isAlive: true as any, revealed: false as any } }); }));
                    }
            ...
            <div>
              <label className="block text-xs text-g500 mb-1">氛围</label>
              <select value={atmosphere} onChange={e => setAtmosphere(e.target.value)} className="w-full sm:w-48 px-3 py-2 text-sm border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400 focus:border-transparent bg-white">
          ...
          <section className="bg-white rounded-r-xl border-g200 p-s6 space-y-4">
            <h2 className="text-lg font-semibold text-g900 flex items-center gap-2"><SectionNum n={2} />主持人</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-g500 mb-1">名称</label>
                <input type="text" value={hostName} onChange={e => setHostName(e.target.value)} className="w-full px-3 py-2 text-sm border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400 focus:border-transparent" />
              </div>
              <div>
                <label className="block text-xs text-g500 mb-1">主持风格</label>
                <input type="text" value={hostStyle} onChange={e => setHostStyle(e.target.value)} className="w-full px-3 py-2 text-sm border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400 focus:border-transparent" />
              </div>
              <div>
                <label className="block text-xs text-g500 mb-1">模式</label>
                <select value={hostMode} onChange={e => setHostMode(e.target.value as any)} className="w-full px-3 py-2 text-sm border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400 focus:border-transparent bg-white">
                  <option value="visible">可见（参与发言）</option>
                  <option value="invisible">不可见（仅控场）</option>
                  <option value="user">用户（你作为主持人）</option>
                </select>
                <p className="text-[11px] text-g400 mt-1">AI 主持人默认使用上帝/裁判视角读取全部秘密，但公开发言不得直接泄露。</p>
              </div>
              <div>
          ...
          <section className="bg-white rounded-r-xl border-g200 p-s6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-g900 flex items-center gap-2"><SectionNum n={3} />阵营<span className="text-xs font-normal text-g400 ml-1">（可选）</span></h2>
              <button type="button" onClick={addTeam} className="inline-flex items-center gap-1 px-2 py-1 text-xs text-p600 hover:bg-p50 rounded-r-lg transition-colors"><Plus className="w-3.5 h-3.5" />添加</button>
            </div>
            {teams.length === 0 && <p className="text-xs text-g400">暂无阵营，角色将使用默认颜色</p>}
            {teams.map((t, i) => (
              <div key={t.id} className="flex items-center gap-3">
                <input type="text" value={t.name} onChange={e => { const n = [...teams]; n[i].name = e.target.value; setTeams(n); }} className="flex-1 px-3 py-2 text-sm border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400 focus:border-transparent" placeholder="阵营名称" />
                <div className="flex gap-1.5">
                  {TEAM_COLORS.map(c => (
                    <button key={c} type="button" onClick={() => { const n = [...teams]; n[i].color = c; setTeams(n); }} className={`w-5 h-5 rounded-md border-2 ${t.color === c ? 'border-g800 scale-110' : 'border-transparent'}`} style={{ background: c }} />
                  ))}
                </div>
                <button type="button" onClick={() => removeTeam(i)} className="text-g400 hover:text-error"><Trash2 className="w-4 h-4" /></button>
              </div>
            ))}
          </section>
          ...
          <section className="bg-white rounded-r-xl border-g200 p-s6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-g900 flex items-center gap-2"><SectionNum n={4} />角色列表</h2>
              <button type="button" onClick={addCharacter} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-p600 hover:bg-p50 rounded-r-lg transition-colors"><Plus className="w-4 h-4" />添加角色</button>
            </div>
            <p className="text-xs text-g400">至少 2 个角色。每个角色可选择不同的 AI 模型；高级选项里可配置隐藏身份、私密目标和初始策略。</p>
                  ...
                  {characters.map((c, i) => {
                    ...
                    <Fragment key={c.id}>
                      <tr className="border-b border-g100">
                        <td className="py-2 px-2"><input type="text" value={c.name} onChange={e => updateCharacter(i, 'name', e.target.value)} placeholder="名称" className="w-full px-2 py-1.5 text-sm border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400" /></td>
                        <td className="py-2 px-2"><input type="text" value={c.role} onChange={e => updateCharacter(i, 'role', e.target.value)} placeholder="身份" className="w-full px-2 py-1.5 text-sm border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400" /></td>
                        ...
                        <td className="py-2 px-2">
                          <div className="flex items-center gap-1">
                            <button type="button" onClick={() => setAdvancedOpen(prev => ({ ...prev, [i]: !prev[i] }))} className="text-g400 hover:text-g600" title="高级选项">
                              {advancedOpen[i] ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                            </button>
                            <button type="button" onClick={() => removeCharacter(i)} className="text-g400 hover:text-error"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                      {advancedOpen[i] && (
                        <tr className="bg-g50">
                          <td colSpan={6} className="py-4 px-4">
                            <div className="space-y-4 text-xs">
                              <div>
                                <p className="font-medium text-g700 mb-2">公开人设补充</p>
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                  <div><label className="text-g500">立场</label><input type="text" value={c.stance || ''} onChange={e => updateCharacter(i, 'stance', e.target.value)} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400" /></div>
                                  <div><label className="text-g500">说话风格</label><input type="text" value={c.style || ''} onChange={e => updateCharacter(i, 'style', e.target.value)} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400" /></div>
                                  <div><label className="text-g500">动机</label><input type="text" value={c.motivation || ''} onChange={e => updateCharacter(i, 'motivation', e.target.value)} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400" /></div>
                                  <div><label className="text-g500">专业领域</label><input type="text" value={c.expertise || ''} onChange={e => updateCharacter(i, 'expertise', e.target.value)} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400" /></div>
                                  <div><label className="text-g500">人物关系</label><input type="text" value={c.relationship || ''} onChange={e => updateCharacter(i, 'relationship', e.target.value)} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400" /></div>
                                  <div><label className="text-g500">限制条件</label><input type="text" value={c.constraints || ''} onChange={e => updateCharacter(i, 'constraints', e.target.value)} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400" /></div>
                                </div>
                              </div>

                              <div>
                                <p className="font-medium text-g700 mb-2">隐藏身份 / 私密信息</p>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                  ...
                                  <div>
                                    <label className="text-g500">公开目标</label>
                                    <input type="text" value={secret.publicGoal} onChange={e => updateSecret(i, 'publicGoal', e.target.value)} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400" />
                                  </div>
                                  <div>
                                    <label className="text-g500">私密目标</label>
                                    <textarea value={secret.privateGoal} onChange={e => updateSecret(i, 'privateGoal', e.target.value)} rows={2} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400 resize-none" />
                                  </div>
                                  <div>
                                    <label className="text-g500">已知秘密 <span className="text-g400">每行一个</span></label>
                                    <textarea value={toLines(secret.knownSecrets)} onChange={e => updateSecret(i, 'knownSecrets', parseLines(e.target.value))} rows={2} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400 resize-none" />
                                  </div>
                                </div>
                                <div className="flex flex-wrap gap-4 mt-2">
                                  <label className="flex items-center gap-2 text-g600"><input type="checkbox" checked={secret.isAlive} onChange={e => updateSecret(i, 'isAlive', e.target.checked)} />在场</label>
                                  <label className="flex items-center gap-2 text-g600"><input type="checkbox" checked={secret.revealed} onChange={e => updateSecret(i, 'revealed', e.target.checked)} />身份已公开</label>
                                </div>
                              </div>

                              <div>
                                <p className="font-medium text-g700 mb-2">初始记忆 / 策略</p>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                  <div>
                                    <label className="text-g500">私有记忆 <span className="text-g400">每行一个</span></label>
                                    <textarea value={toLines(memory.privateMemory)} onChange={e => updateMemory(i, 'privateMemory', parseLines(e.target.value))} rows={2} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400 resize-none" />
                                  </div>
                                  <div>
                                    <label className="text-g500">公开记忆 <span className="text-g400">每行一个</span></label>
                                    <textarea value={toLines(memory.publicMemory)} onChange={e => updateMemory(i, 'publicMemory', parseLines(e.target.value))} rows={2} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400 resize-none" />
                                  </div>
                                  <div className="sm:col-span-2">
                                    <label className="text-g500">初始策略计划</label>
                                    <textarea value={memory.strategyPlan} onChange={e => updateMemory(i, 'strategyPlan', e.target.value)} rows={2} className="w-full mt-1 px-2 py-1 border-g200 rounded-r focus:outline-none focus:ring-1 focus:ring-p400 resize-none" />
                                  </div>
                                </div>
                              </div>
                            </div>
          ...
          {/* ===== 5. 规则 ===== */}
          <section className="bg-white rounded-r-xl border-g200 p-s6 space-y-4">
            <h2 className="text-lg font-semibold text-g900 flex items-center gap-2"><SectionNum n={5} />讨论规则</h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm text-g700">不预设轮数</span>
                </label>
                {!unlimitedRounds && (
                  <div className="flex items-center gap-2">
                    <input type="number" value={roundCount} onChange={e => setRoundCount(Math.max(2, Math.min(50, parseInt(e.target.value) || 3)))} min={2} max={50} className="w-16 px-2 py-1.5 text-sm text-center border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400" />
                    <span className="text-sm text-g500">轮</span>
                  </div>
                )}
                {unlimitedRounds && <p className="text-xs text-g400 mt-1">不限轮数，最多 999 轮安全上限，可随时停止</p>}
              </div>
              <div>
                <label className="block text-xs text-g500 mb-1">发言顺序</label>
                <select value={speakOrder} onChange={e => setSpeakOrder(e.target.value as any)} className="w-full px-3 py-2 text-sm border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400 bg-white">
              ...
              <div>
                <label className="block text-xs text-g500 mb-1">字数上限</label>
                <input type="number" value={maxSpeechLength} onChange={e => setMaxSpeechLength(Math.max(50, Math.min(1000, parseInt(e.target.value) || 300)))} min={50} max={1000} className="w-20 px-2 py-1.5 text-sm text-center border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400" />
              </div>
            </div>
            <div>
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={scoringEnabled} onChange={e => setScoringEnabled(e.target.checked)} className="rounded" />
                <span className="text-sm text-g700">启用评分</span>
              </label>
            </div>
            <div>
              <label className="block text-xs text-g500 mb-1">禁止话题</label>
              <textarea value={forbiddenTopics} onChange={e => setForbiddenTopics(e.target.value)} placeholder="每行一个话题，讨论中将避开这些内容" rows={2} className="w-full px-3 py-2 text-sm border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400 resize-none" />
            </div>
          </section>
          ...
          <section className="bg-white rounded-r-xl border-g200 p-s6 space-y-4">
            ...
            <div>
              <label className="block text-xs text-g500 mb-1">目标描述</label>
              <textarea value={goalDesc} onChange={e => setGoalDesc(e.target.value)} placeholder="描述本次讨论希望达成的具体目标..." rows={2} className="w-full px-3 py-2 text-sm border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400 resize-none" />
            </div>
            <div>
              <label className="block text-xs text-g500 mb-1">成功标准 <span className="text-g400">（可选）</span></label>
              <input type="text" value={goalCriteria} onChange={e => setGoalCriteria(e.target.value)} placeholder="如何衡量讨论是否成功？" className="w-full px-3 py-2 text-sm border-g300 rounded-r-lg focus:outline-none focus:ring-2 focus:ring-p400" />
            </div>
          </section>

          {error && <div className="p-4 bg-error/10 border-error/30 rounded-r-xl text-sm text-error">{error}</div>}

          {/* Submit bar */}
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-g200 p-4 flex justify-between items-center z-40">
            <span className="text-xs text-g400">共 {characters.filter(c => c.name.trim()).length} 个角色 · {unlimitedRounds ? '不限轮数' : `${roundCount} 轮`}</span>
            <button type="submit" disabled={saving} className="flex items-center gap-2 px-8 py-3 bg-p600 text-white rounded-r-xl font-medium hover:bg-p700 transition-colors shadow-md shadow-p200 disabled:opacity-50">
